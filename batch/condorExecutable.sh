...
# Run babyMaker
./runBTagEffMaker "$NAME" $NEVENTS $NUMBER "./"
ls -l `pwd`/bTagEfficiency__${NAME}_${NUMBER}.root

# Copy the output to the output directory via lcg-cp
#  This preserves grid functionality
echo "copying.  LS is: "
ls -l bTagEfficiency__${NAME}_${NUMBER}.root

lcg-cp -b -D srmv2 --vo cms --connect-timeout 2400 --verbose file://`pwd`/bTagEfficiency__${NAME}_${NUMBER}.root srm://bsrm-3.t2.ucsd.edu:8443/srm/v2/server?SFN=${COPYDIR}/bTagEfficiency__${NAME}_${NUMBER}.root
